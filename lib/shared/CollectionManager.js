...
class CollectionManagerServer {
  ...
  sendAll (connection) {
    _.each(this.collections, (collection, collectionId) =>
      connection.emit('collection.init', collectionId, collection.indexed())
    );
  }
...
module.exports = {
  Server: CollectionManagerServer,
  Client: CollectionManagerClient
};
